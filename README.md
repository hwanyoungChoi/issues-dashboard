...
| 게시글 삭제/수정/등록 시 목록을 갱신하기 위해 invalidateQueries 또는 refetch를 호출해도, 목록이 즉시 갱신되지 않아 사용성 이슈가 발생            | GitHub Search API 결과가 실시간으로 반영되지 않고, 서버에서 최신 데이터가 오기까지 지연이 발생함을 확인함                                | 해결보류 - 낙관적 업데이트(optimistic update)로 프론트엔드에서 즉시 UI를 갱신했으나, 1) 삭제 시 서버 데이터가 완전히 반영되기 전까지는 목록이 10개가 아닌 9개로 노출되는 점, 2) 지연 시간이 예측 불가하여 언제 갱신할지 애매한 점, 3) 등록/수정/삭제 등 각 상황별로 후속 처리 코드가 복잡해지는 점 등의 한계가 있었음 | 데이터 일관성을 위해 유의미한 수준의 지연 이슈가 발생할 수 있음을 경험했고, API의 특성을 파악해야함을 깨달음. 낙관적 업데이트를 처음 다뤄볼 수 있었음.       |
