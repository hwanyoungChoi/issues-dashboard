...
| 게시글 삭제/수정/등록 시 목록을 갱신하기 위해 invalidateQueries 또는 refetch를 호출해도, 목록이 즉시 갱신되지 않아 사용성 이슈가 발생            | GitHub Search API 결과가 실시간으로 반영되지 않고, 서버에서 최신 데이터가 오기까지 지연이 발생함을 확인함                                | 해결: github issues 페이지를 참고하였고 이슈 닫힘 처리 직후에는 목록에서 제거가 아니라 닫힘 표시를 해주는 것을 알았습니다. 또 이슈 등록/닫힘 시 목록으로 이동시키는게 아니라 해당 상세 페이지를 보여주는 것을 알았습니다. 이에 아이디어를 얻었습니다., 게시글 목록에 즉각적으로 추가, 삭제되지는 않지만 state는 즉시 변경되겠다고 예상했고 맞아 떨어져 github issues 페이지에서의 처리방식과 동일하게 개발해서 해결했습니다. 해결시도: 낙관적 업데이트(optimistic update)로 프론트엔드에서 즉시 UI를 갱신했으나, 1) 삭제 시 서버 데이터가 완전히 반영되기 전까지는 목록이 10개가 아닌 9개로 노출되는 점, 2) 지연 시간이 예측 불가하여 언제 갱신할지 애매한 점, 3) 등록/수정/삭제 등 각 상황별로 후속 처리 코드가 복잡해지는 점 등의 한계가 있었음 | 데이터 일관성을 위해 유의미한 수준의 지연 이슈가 발생할 수 있음을 경험했고, API의 특성을 파악해야함을 깨달음. 낙관적 업데이트를 처음 다뤄볼 수 있었음.       |
...
게시글 조작 후 목록으로 이동시켜주면서, 갱신된 목록을 보여주도록 처리하려했으나 GitHub API에서 일정 지연 시간이 있다는 것을 알게되었습니다. 낙관적 업데이트로 각 케이스별로 처리해주기에는 코드가 복잡해지는 면이 있었고, 임의의 delay이후 갱신요청하기에는 정확한 delay를 예측할 수 없으면서 UX측면에서도 손실이 있었습니다.

혹시 목록만 즉각 변경되지않고 state는 변경되는 것 아닌가? 라는 예측에서 확인해본 결과 맞았고 github issues 페이지에서도 state로 처리하는 것으로 보여서 동일한 방식으로 개발했습니다. 1) 삭제 시 목록에서는 즉시 삭제되지 않더라도 삭제 표시 및 이벤트 방지하기, 2) 등록/수정 시 목록페이지를 바로 보여주지 말고 해당 상세페이지로 이동시켜서 목록 갱신 시간 확보하기
